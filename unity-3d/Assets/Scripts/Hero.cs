...
// Dynamic 3D curved slash arc ribbon + imported particle slash
  ...
  public static CharacterVisual Create(string role,Transform parent,float height,Color color){
   ...
   // DogKnight's humanoid takes need the avatar generated by its importer.
   if(role=="DogKnight"&&v.animator&&!v.animator.avatar){
    foreach(var avatar in Resources.LoadAll<Avatar>("Characters/DogKnight"))if(avatar.isHuman&&avatar.isValid){v.animator.avatar=avatar;break;}
   }
