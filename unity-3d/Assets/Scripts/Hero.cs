...
// Dynamic 3D curved slash arc ribbon + imported particle slash
  ...
  public static CharacterVisual Create(string role,Transform parent,float height,Color color){
   ...
   // Humanoid takes need the avatar generated by their importer.
   if(v.animator&&!v.animator.avatar){
    foreach(var avatar in Resources.LoadAll<Avatar>("Characters/"+role)){if(avatar.isHuman&&avatar.isValid){v.animator.avatar=avatar;break;}}
    if(!v.animator.avatar){foreach(var avatar in Resources.LoadAll<Avatar>("Characters/"+role)){if(avatar.isValid){v.animator.avatar=avatar;break;}}}
   }
