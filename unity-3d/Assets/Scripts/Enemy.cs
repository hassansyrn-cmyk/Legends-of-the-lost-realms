...
namespace LostRealms {
 public class Enemy:MonoBehaviour {
  ...
  void CommitAttack(){var g=RealmGame.I;g.Sound(Boss?"boss":"enemy_dash");if(Boss){int attack=(attackCount+Kind)%3;
   ...
   else{Vector3 d=g.Player.transform.position-transform.position;if(d.magnitude<3&&Vector3.Dot(transform.forward,d.normalized)>.05f)g.Player.Damage(Kind==6?2:1,transform.position);HitSpark.Burst(transform.position+transform.forward,transform.forward,new Color(1,.35f,.18f),8);}
  }
  ...
  public void Hit(float damage,int power,bool elemental,Vector3 knockDir=default){
   ...
   ApplyDamage(damage*(Boss&&state==State.Recover?1.35f:1));
  ...
  void ApplyDamage(float amount){
   ...
   if(Health<=0){
    state=State.Dead;if(warning)Destroy(warning);Visual.Restart("death");
 ...
 public class EnemyBolt:MonoBehaviour {
  ...
  void Update(){
   ...
   if(Physics.Raycast(transform.position,direction,out var hit,step,~0,QueryTriggerInteraction.Ignore)){
    if(hit.collider.GetComponent<Hero>())g.Player.Damage(damage,transform.position);
   ...
   if(Vector3.Distance(transform.position,g.Player.transform.position+Vector3.up*.8f)<.7f){
    g.Player.Damage(damage,transform.position);
    HitSpark.Burst(transform.position,-direction,new Color(1f,.4f,.15f),8);
    Destroy(gameObject);
 ...
 public class StrikeZone:MonoBehaviour {
  ...
  void Update(){
   ...
   if(new Vector2(d.x,d.z).magnitude<radius&&Mathf.Abs(d.y)<1.2f)g.Player.Damage(damage,transform.position);
