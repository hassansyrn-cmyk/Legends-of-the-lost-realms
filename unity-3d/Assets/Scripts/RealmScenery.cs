   ...
   foreach(Transform island in world.transform){
    ...
    if(width>7){
     for(int side=-1;side<=1;side+=2){
      ...
      for(int n=0;n<5;n++){float z=(float)(rng.NextDouble()-.5)*(length-1);Vector3 r=new Vector3(side*(width*.5f-.25f),.22f,z);var b=Art.Shape("Weathered edge rock",PrimitiveType.Sphere,r,new Vector3(.45f,.35f,.65f),Color.white,island);b.GetComponent<Renderer>().sharedMaterial=rock;}
     }
    }
