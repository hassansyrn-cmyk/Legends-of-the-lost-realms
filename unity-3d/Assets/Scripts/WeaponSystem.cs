...
namespace LostRealms {
 ...
 public static class WeaponCatalog {
  ...
  public static GameObject CreateModel(WeaponId id,Transform parent){
   ...
   // Supplied weapon FBX ships without usable materials (white mesh), so bind
   // steel to the largest part (blade) and dark grip elsewhere — but keep any
   // imported textured material untouched.
   foreach(var renderer in renderers){var m=renderer.sharedMaterial;if(m!=null&&m.mainTexture!=null)continue;renderer.sharedMaterial=renderer==blade?BladeMaterial():GripMaterial();}
   return model;
  }
